...
def generate_pieces(file_path, piece_size=PIECE_SIZE*1024):
    pieces = []
...
def generate_torrent_json(directory, piece_length=PIECE_SIZE):
    torrent_data = {
        "tracker": TRACKER_URL,
        "info": {
    ...
    if os.path.isfile(directory):  # For a single file
        ...
        for root, _, files in os.walk(directory):
            for filename in files:
                ...
                pieces = generate_pieces(file_path, PIECE_SIZE)
                torrent_data["info"]["files"].append({
                    "length": file_size,
                    "path": relative_path,
                    "pieces": pieces
