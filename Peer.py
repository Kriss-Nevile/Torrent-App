...
class Peer:
    ...
    def __init__(self, port, torrent_file="", seeder=False):
        ...
        # used to keep track of the chunks downloaded, we if need to get size, we can use len(downloaded)
        self.chunks_downloaded = []   # payload: {"filepath": path/name, "piece_index": }
        self.left = 0
        self.chunks_left = []
        self.duplicate = 0
        ...
        # A local storage stores chunks of files temporaryly
        self.local_storage = []
        ...
        Sample torrent_data:
        {
            "tracker": "https://simple-like-torrent-application.vercel.app/",
            "info": {
                "name": "data",
                "piece length": 524288,
                "files": [
                    {
                        "length": 152449,
                        "path": [
                        "Assignment 1-Network Application P2P File Sharing.pdf"
                        ],
                        "pieces": [
                        {
                            "index": 0,
            ...
            try:
                ...
                with self.general_update_lock:
                    with peer_obj.receive_lock:
                        # Create and send a request message to another peer
                        # Format of a request payload: {"filepath": path/name, "piece_index": }
            ...
            if readable:
                try:
                    ...
                    if message_type == 'keep-alive':
                        ...
                        # Format of a have payload: {"filepath": path/name, "piece_index": }
                        try:
                            filepath = payload.get('filepath')
                            piece_index = payload.get('piece_index')
                            print(f"REC: Received have message for file {filepath} with piece index {piece_index}")
                            if filepath and piece_index is not None:
                                peer_obj.available_chunks.append({'filepath': filepath, 'piece_index': piece_index})
                        except Exception as e:
                            print(f"ERROR: Failed to process have message: {e}")
                    
                    elif message_type == 'request':
                        # Receive a request from source peer
                        # Format of a request payload: {"filepath": path/name, "piece_index": }
                        try:
                        ...
                        # Receive piêc data (chunk data)
                        # Format of payload from sending data message:
                        # {"filepath": path/name, "piece_index": , "chunk_data": binary-data}
                        ...
                        print(f"REC: Received CHUNK DATA for file '{filepath}' with piece index {piece_index} from peer with ID: {peer_obj.ID}")

                        with self.general_update_lock:
                            if chunk_metadata in self.chunks_left:
                                # Save the chunk data to local_sotrage
    ...
    # Adds a new file to the local storage with its associated pieces.
    ...
    def Main(self):
        # Give me some sample usages at here
        # TODO
        pass
